...
const AgentReceivedEnquiries = () => {
  ...
  useEffect(() => {
    ...
    if (loggedInUser && loggedInUser.role === 'agent') {
      fetchAgentEnquiries(loggedInUser.id);
    } else {
      toast.error('You must be logged in as an agent to view this page.');
    }
  }, []);

  const fetchAgentEnquiries = async (agentId) => {
    try {
      const response = await axios.get(`http://localhost:5000/api/properties/agent/enquiries/${agentId}`);
      setEnquiries(response.data.enquiries);
    } catch (error) {
      console.error('Error fetching agent enquiries:', error);
      toast.error('Error fetching agent enquiries');
    }
  };
  ...
  const handleResponseSubmit = async (enquiryId) => {
    try {
      ...
      setResponse('');
      fetchAgentEnquiries(user.id); // Refresh enquiries
    } catch (error) {
  ...
  return (
    <div className="flex flex-wrap justify-around gap-6 bg-white p-8 rounded-lg shadow-lg mb-10">
      ...
      <div className="w-full mt-10">
        <h3 className="text-xl font-semibold mb-4">All Received Enquiries</h3>
                ...
                enquiries.map((enquiry) => (
                  <tr key={enquiry._id}>
                    ...
                    <td className="py-3 px-4 border-b border-gray-200">
                      ...
                      ) : enquiry.property.owner === user.id ? (
...
export default AgentReceivedEnquiries;
