...
const AdminReceivedEnquiries = () => {
  ...
  const fetchAdminEnquiries = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/properties/admin/enquiries');
      setEnquiries(response.data.enquiries);
    } catch (error) {
      console.error('Error fetching admin enquiries:', error);
      toast.error('Error fetching admin enquiries');
    }
  };
  ...
  const handleResponseSubmit = async (enquiryId) => {
    try {
      ...
      setResponse('');
      fetchAdminEnquiries(); // Refresh enquiries
    } catch (error) {
...
export default AdminReceivedEnquiries;
